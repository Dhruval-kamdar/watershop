<?php
	...
	if($err!='')
	{
		...
		orderId, invoiceNo, productDetails, grandTotal, orderStatus, orderTimestamp, deliveryTimestamp, subTotal, vat, discount, grandTotal, o.purchasePoints, address FROM `tbl_orders` o INNER JOIN tbl_customers c ON o.custId=c.custId WHERE o.custId='".$custId."' AND (orderStatus='6' OR orderStatus>6) $where ORDER BY orderId DESC");
		$rows = $conn->records_to_array($sql);
	
		$sql1 = $conn->get_record_set("SELECT 
		orderId, invoiceNo, productDetails, grandTotal, orderStatus, orderTimestamp, deliveryTimestamp, subTotal, vat, discount, grandTotal, o.purchasePoints, address FROM `tbl_orders` o INNER JOIN tbl_customers c ON o.custId=c.custId WHERE o.custId='".$custId."' AND (orderStatus='6' OR orderStatus>6) $where ORDER BY orderId DESC $limit");
