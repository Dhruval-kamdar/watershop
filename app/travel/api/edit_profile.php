<?php
	...
	if($_POST['address']!='')
		$postdata['address'] = mysqli_real_escape_string($dbConn,$_POST['address']);
	...
	if(empty($postdata['phone']))
	{
			...
			{
				/*$cont = $data->select( "tbl_continents" , "*", array("continent_id"=>$row1['continent_id']));
				$row1['continent_name'] = $cont[0]['continent_name'];
				$cout = $data->select( "tbl_countries" , "*", array("country_id"=>$row1['country_id']));
				$row1['country_name'] = $cout[0]['country_name'];*/
				...
				if($row1['full_name']=='' || $row1['email_id']=='')
