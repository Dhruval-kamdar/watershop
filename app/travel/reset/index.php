...
if(isset($_POST['new_pass']) && isset($_POST['confirm_new_pass'])) {
	    ...
	    if(!$show_form) {
	   	...
	   	if(!$show_form) {
	   		...
	   		$query = "UPDATE tbl_users SET password='$password',passResetToken='' WHERE custId = $custId";
	   		if(mysqli_query($dbConn,$query)) {
	   			//mysqli_query($dbConn,"DELETE FROM tbl_users WHERE custId=$custId AND token = '$token'");
...
if($show_form) {
	$token = ($_GET['t']);
	$query = "SELECT custId FROM tbl_users WHERE passResetToken = '$token'";
