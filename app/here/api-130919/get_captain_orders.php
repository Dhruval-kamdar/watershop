...
require_once('include/thumb.php');
	...
	if($rows1[0]['captain_status']=='Pending')
	{
		...
		$jsonArray['Message']="Your request is pending for approval.";
	}
	elseif($rows1[0]['captain_active']=='0')
	{
		$jsonArray['Success']='0';
		$jsonArray['Message']=$lang["YOUR_ACCOUNT_DEACTIVATED"];
		$jsonArray['Message']="You are deactivated. Please contact administrator.";
	}
	elseif($rows1[0]['wallet_balance'] <='0')
	{
		$jsonArray['Success']='0';
		$jsonArray['Message']=$lang["CHARGE_YOUR_WALLET"];
		$jsonArray['Message']="You can't receive order. Please charge your wallet.";
	}
	else
	{	
		...
		else
		{
			...
			$jsonArray['Message']="No new order available";
		}
	}
