<?php
	...
	if($_POST['grand_total']!=$postdata['grand_total'])
	{
		...
		$jsonArray['Message']='Incorrect grand total amount.';
		show_output($jsonArray);
	}
	...
	{	
		...
		if(!empty($row[0]))
		{
			...
			elseif($discount!=$postdata['discount'])
			{
				$jsonArray['Success']='0';
				$jsonArray['Message']=$lang["SUCCESSFUL"];
				$jsonArray['Message']='Discount amount is not correct.';
				show_output($jsonArray);
			}
		}
		else
	...
	if(!empty($rows))
	{
			if($postdata['payment_type']=='In System Transfer')
			{
				if($rows[0]['wallet_balance'] < $postdata['grand_total'])
				{
					$jsonArray['Success']='0';
					$jsonArray['Message']=$lang["SUCCESSFUL"];
					$jsonArray['Message']='Your wallet balance is not enough.';
					show_output($jsonArray);
				}
				...
				if($remain_balance < $postdata['grand_total'])
				{
					$jsonArray['Success']='0';
					$jsonArray['Message']=$lang["SUCCESSFUL"];
					$jsonArray['Message']='Your wallet balance is not enough due to you have already system transfer type order in progress.';
					show_output($jsonArray);
				}
			}
