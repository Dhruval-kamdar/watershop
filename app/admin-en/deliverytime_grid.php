...
$sql = "SELECT * ";
$sql.=" FROM tbl_delivery_times WHERE 1=1 AND (isDeleted='0')";
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	$nestedData[] = "<div class='btn-group'>
											  ...
											  <ul class='dropdown-menu primary' role='menu'>
												<li><a onclick=updateStatus('".$row["timeId"]."','active') >Active</a></li>
												<li><a onclick=updateStatus('".$row["timeId"]."','deactive') >Deactive</a></li>
											  </ul>
											</div>";
	...
	<a data-id="'.$row["timeId"].'" class="btnDelete btn btn-xs btn-danger" >Delete</a>';
	$data[] = $nestedData;
	$i++;
}
$json_data = array(
