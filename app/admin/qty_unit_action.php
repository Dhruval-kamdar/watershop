...
if($_REQUEST['action']=='active')
{
	...
	if(!empty($data_id_array)) {
		foreach($data_id_array as $id) {
			$sql = "UPDATE tbl_qty_units SET $field ";
			$sql.=" WHERE qtyUnitId = '".($id)."'";
			$query=mysqli_query($conn, $sql) or die("qty_unit_action.php: delete citys");
		}
	}
}
...
{	
	...
	if(trim($data_ids1)!="")
	{
		mysqli_query($conn, "UPDATE tbl_qty_units SET deliveryFee='".$row['value']."'
		WHERE qtyUnitId IN (".$data_ids1.")");
	}
	if(trim($data_ids)!="")
	{
		mysqli_query($conn, "UPDATE tbl_qty_units SET $field WHERE qtyUnitId IN (".$data_ids.")");
	}
