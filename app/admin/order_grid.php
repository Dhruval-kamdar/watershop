...
if($_SESSION['custId_orderlist']!="")
{
	$custId = $converter->decode($_SESSION['custId_orderlist']);
	$where = " custId='".$custId."'";
}
else
{
...
$sql = "SELECT * ";
$sql.=" FROM tbl_orders  WHERE $where";
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	{
		$style = 'background-color: orange;color: black';//for اعتماد
	}
	else
	{
	...
	$invoiceUrl = "../invoice?id=".$encodedId;
	...
	$cancelOrder = '&nbsp;|&nbsp;<a class="cancelDialog" data-id="'.$encodedId.'" href="#cancelModel" data-toggle="modal" style="text-decoration:underline">إلغاء</a>';
	if($_SESSION[SESSION_NAME . "role"]!="account_management" ) {
	if($row["orderStatus"]=='1')
	$statusAction =	'<br><a  href="order_list.php?id='.$encodedId.'&action='.$encodedStatus.'&code='.$converter->encode("2").'" style="text-decoration:underline" >اعتماد</a>'.$cancelOrder;
	else if($row["orderStatus"]=='2')
	$statusAction =	'<br><a  href="order_list.php?id='.$encodedId.'&action='.$encodedStatus.'&code='.$converter->encode("3").'" style="text-decoration:underline" >تجهيز</a>'.$cancelOrder;	
	else if($row["orderStatus"]=='3')
	$statusAction =	'<br><a  href="order_list.php?id='.$encodedId.'&action='.$encodedStatus.'&code='.$converter->encode("4").'" style="text-decoration:underline" >في الطريق</a>'.$cancelOrder;	
	else if($row["orderStatus"]=='4')
	$statusAction =	'<br><a  href="order_list.php?id='.$encodedId.'&action='.$encodedStatus.'&code='.$converter->encode("5").'" style="text-decoration:underline" >عند الباب</a>'.$cancelOrder;	
	else if($row["orderStatus"]=='5')
	$statusAction =	'<br><a  href="order_list.php?id='.$encodedId.'&action='.$encodedStatus.'&code='.$converter->encode("6").'" style="text-decoration:underline" >اكتمال</a>'.$cancelOrder;		
	else
	$statusAction ='';
	}
	...
	$nestedData[] = '<span style="margin-left:10px">'.'<a class="ViewDialogCust" data-id="'.$converter->encode($row["custId"]).'" href="#viewModelCust" data-toggle="modal">'.$row["custId"].'</a></span>';
	...
	$nestedData[] = '<span style="margin-left:10px"><a class="ViewDialog btn btn-xs btn-success" data-id="'.$encodedId.'" href="#viewModel" data-toggle="modal">عرض</a>
	<a class="btn btn-xs btn-success" href="'.$invoiceUrl.'" title="Invoice" target="_blank" > فاتورة </a></span>';
	$data[] = $nestedData;
	$i++;
}
$json_data = array(
