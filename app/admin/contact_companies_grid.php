...
$sql = "SELECT c.id ";
$sql.=" FROM tbl_contact_companies c WHERE type='contactCompany' AND isDeleted='0'";
$query=mysqli_query($conn, $sql) or die("contact_companies_grid.php: get employees1");
...
$query=mysqli_query($conn, $sql) or die("contact_companies_grid.php: get employees2");
$totalFiltered = mysqli_num_rows($query); 
$sql.=" ORDER BY ". $columns[$requestData['order'][0]['column']]."   ".$requestData['order'][0]['dir']."  LIMIT ".$requestData['start']." ,".$requestData['length']."   ";
$query=mysqli_query($conn, $sql) or die("contact_companies_grid.php: get employees3");
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	$editurl = "add_contact_companies.php" . $urltoadd . ($urltoadd != "" ? "&id=" . $encodedId : "?id=" . $encodedId);
