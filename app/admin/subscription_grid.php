...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	if($row["status"] == '0')
	{
		$style = 'background-color: orange;color: black';//for معلّق
	}
	...
	{
		$style = 'background-color: green;color: black';//for اعتماد
	}
	else
	{
	...
	if($row["status"]=='0')
	$statusAction =	'<br><a  href="subscriptions.php?id='.$encodedId.'&action='.$converter->encode("approveSubscription").'" style="text-decoration:underline" >اعتماد</a>&nbsp;|&nbsp;<a class="cancelDialog" data-id="'.$converter->encode($row["subId"]).'" href="#cancelModel" data-toggle="modal" style="text-decoration:underline">إلغاء</a>';
	...
	$nestedData[] = ($row['status']=='0')?"معلّق".$statusAction:($row['status']=='1'?"نشط":"Rejected");
	
	if($row['status']=='1')
		$cancle = ' <a class="cancelDialog btn btn-xs btn-danger" data-id="'.$converter->encode($row["subId"]).'" href="#cancelModel" data-toggle="modal" >إلغاء</a>';
	else
		$cancle = ' ';
		
	if($row['status']=='1')
		$edit = ' <a href="'.$editurl.'" title="تصحيح" ><button class="btn btn-xs btn-success">تعديل</button></a>';
	else
		$edit = ' ';
	$nestedData[] = ' <a class="ViewDialog btn btn-xs btn-success" data-id="'.$converter->encode($row["subId"]).'" href="#viewModel" data-toggle="modal"><i class="icon-trash icon-white"></i>عرض</a> '.$edit. $cancle;
