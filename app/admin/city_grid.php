...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	$nestedData[] = "<div class='btn-group'>
											  ...
											  <ul class='dropdown-menu primary' role='menu'>
												<li><a onclick=updateStatus('".$row["cityId"]."','active') >نشط</a></li>
												<li><a onclick=updateStatus('".$row["cityId"]."','deactive') >غير نشط</a></li>
											  </ul>
											</div>";
	...
	<a data-id="'.$row["cityId"].'" class="btnDelete btn btn-xs btn-danger" >حذف</a>';
	$data[] = $nestedData;
	$i++;
}
$json_data = array(
